...
func TestAnalyze_TechPreference_ExplicitPrefer(t *testing.T) {
	memories := []*db.Memory{
		{ID: "1", Content: "I always use Go for backend services, written in Go", Speaker: "user"},
		{ID: "2", Content: "I prefer to use Go for CLI tools, switched to golang", Speaker: "user"},
		{ID: "3", Content: "Built the new service in Go", Speaker: "user"},
		{ID: "4", Content: "Go module setup for the proxy", Speaker: "user"},
	}
...
func TestAnalyze_TechPreference_Avoids(t *testing.T) {
	memories := []*db.Memory{
		{ID: "1", Content: "Don't use Python for this, it's too slow", Speaker: "user"},
		{ID: "2", Content: "Avoid Python for performance-critical paths", Speaker: "user"},
		{ID: "3", Content: "Never use Python in the data pipeline", Speaker: "user"},
		{ID: "4", Content: "Python script for one-off migration", Speaker: "user"},
	}
...
func TestAnalyze_BelowThreshold_NoPattern(t *testing.T) {
	memories := []*db.Memory{
		{ID: "1", Content: "Used Docker once", Speaker: "user"},
		{ID: "2", Content: "Something unrelated", Speaker: "user"},
	}
...
func TestAnalyze_DecisionPattern_Security(t *testing.T) {
	memories := []*db.Memory{
		{ID: "1", Content: "Checked the auth token flow before we decided on the API design", Speaker: "user"},
		{ID: "2", Content: "Going with mTLS after reviewing security implications", Speaker: "user"},
		{ID: "3", Content: "Settled on RBAC after evaluating permission models and security", Speaker: "user"},
		{ID: "4", Content: "Something unrelated about lunch", Speaker: "user"},
		{ID: "5", Content: "Regular code review", Speaker: "user"},
	}
