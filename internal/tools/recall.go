...
func (r *Recall) Tool() mcp.Tool {
	return mcp.NewTool("recall",
		...
		mcp.WithString("project", mcp.Description("Filter by a single project/namespace (e.g. 'agent:mybot', 'crew:shared')")),
		mcp.WithArray("projects", mcp.Description("Filter by multiple namespaces — results from any match (e.g. ['agent:alice','crew:shared'])"), mcp.WithStringItems()),
		mcp.WithString("type",
