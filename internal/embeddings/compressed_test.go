...
func TestCompressedEmbed(t *testing.T) {
	...
	// 4-bit = 0.5 bytes per angle, ~192 angles for 384 dims, ~96 bytes + header
...
func TestCompressDecompressRoundtrip(t *testing.T) {
	inner := &stubProvider{dims: 384}
	cp, _ := NewCompressedProvider(inner, 8) // 8-bit for higher fidelity
	...
	// Check cosine similarity > 0.95 for 8-bit
...
func TestCompressedProvider_RegularEmbed(t *testing.T) {
	...
	// Regular Embed should still work (delegates to inner)
